...
class DatabaseManager:
    """データベース操作を抽象化するクラス。USE_SUPABASEフラグに応じて処理を切り替える。"""
    def __init__(self):
        self.use_supabase = USE_SUPABASE and SUPABASE_URL and SUPABASE_SERVICE_KEY
    ...
    async def initialize(self):
        ...
        try:
            db_client = chromadb.PersistentClient(path=DB_PATH)
            print("ChromaDBクライアントの初期化完了（フォールバック）。")
            return True
        except Exception as e:
            print(f"データベース初期化失敗: {e}")
            return False
    ...
    async def add_documents(self, texts: List[str], embeddings: List[List[float]], 
        ...
        if self.use_supabase and supabase:
            ...
            await asyncio.to_thread(
                ...
                ids=[str(uuid.uuid4()) for _ in texts], metadatas=metadata_list
            )
            return len(texts)
    ...
    async def search_documents(self, query_embedding: List[float], collection_name: str, top_k: int):
        if self.use_supabase and supabase:
            try:
                # デバッグ用のログ出力
                print(f"Supabaseでコレクション'{collection_name}'を検索します。")
                
                # pgvectorでのコサイン類似度検索
                # 【重要】引数名をSQLで定義した 'p_...' 形式に修正
    ...
    async def list_collections(self):
        if self.use_supabase and supabase:
            try:
                # コレクション名と件数を取得するRPC関数を呼び出し
                result = supabase.rpc('list_collections_with_counts').execute()
    ...
    async def create_collection(self, name: str):
        # Supabaseではテーブルが一つなので、この操作は実質不要。互換性のために残す。
        if self.use_supabase:
            return {"name": name, "message": "Supabaseではコレクションは動的に扱われます。"}
        else:
            collection = await asyncio.to_thread(db_client.create_collection, name=name)
            return {"name": collection.name, "message": "ChromaDBにコレクションを作成しました。"}
    
    async def delete_collection(self, name: str):
        if self.use_supabase and supabase:
            try:
                supabase.table('documents').delete().eq('collection_name', name).execute()
                return {"message": f"コレクション'{name}'に紐づくドキュメントを削除しました。"}
            except Exception as e:
                raise HTTPException(500, f"コレクション削除失敗: {e}")
        else:
            await asyncio.to_thread(db_client.delete_collection, name=name)
            return {"message": "コレクションを削除しました"}
    ...
    async def get_documents(self, collection_name: str):
        if self.use_supabase and supabase:
            try:
                ...
                sources = list(set(
                    doc['metadata'].get('filename') or doc['metadata'].get('source_url', 'unknown') 
                    for doc in result.data
                ))
                return {"documents": [{"id": src} for src in sorted(sources)], "count": result.count}
            except Exception as e:
            ...
            try:
                ...
                if count == 0: return {"documents": [], "count": 0}
                
                data = await asyncio.to_thread(collection.get, include=['metadatas'])
                sources = sorted(list(set(
                    md.get('filename') or md.get('source_url', 'unknown') for md in data['metadatas']
                )))
                return {"documents": [{"id": src} for src in sources], "count": count}
            except ValueError:
                return {"documents": [], "count": 0}
            except Exception as e:
        ...
        if self.use_supabase and supabase:
            try:
                supabase.table('feedback_logs').insert({
                    'log_id': log_id, 'timestamp': timestamp, 'query': query, 'response': response,
                    'rating': rating, 'category': category, 'has_specific_info': has_specific_info
                }).execute()
            except Exception as e:
                print(f"Supabaseフィードバック保存エラー: {e}。CSVにフォールバックします。")
    ...
    async def get_logs(self):
        if self.use_supabase and supabase:
            try:
                result = supabase.table('feedback_logs').select('*').order('timestamp', desc=True).limit(1000).execute()
                return result.data
            except Exception as e:
                print(f"Supabaseログ取得エラー: {e}。CSVから読み込みます。")
    ...
    async def update_feedback(self, log_id: str, rating: str):
        if self.use_supabase and supabase:
            try:
                result = supabase.table('feedback_logs').update({'rating': rating}).eq('log_id', log_id).execute()
...
# --------------------------------------------------------------------------
# 4. アプリケーションのライフサイクルとFastAPIインスタンス化
# --------------------------------------------------------------------------
...
# (以下、テキスト処理、シナリオ、カテゴリ等のヘルパー関数は変更ないため省略)
# ... (Existing helper functions, KEYWORD_MAP, SCENARIOS etc. remain the same) ...
...
async def chromadb_status():
    ...
    try: 
        db_client.heartbeat(); return {"status": "ok"}
    except Exception as e: 
...
async def list_collections(): return await db_manager.list_collections()

@app.post("/collections", dependencies=[Depends(require_sgu_member)])
async def create_collection_api(req: CreateCollectionRequest):
    if not (name := req.name.strip()): raise HTTPException(400, "Collection name is empty")
    try: return await db_manager.create_collection(name)
    except Exception as e: raise HTTPException(500, f"Failed to create collection: {e}")
...
async def delete_collection_api(collection_name: str):
    try: return await db_manager.delete_collection(collection_name)
    except Exception as e: raise HTTPException(500, f"Failed to delete collection: {e}")

@app.get("/collections/{collection_name}/documents", dependencies=[Depends(require_sgu_member)])
async def get_documents(collection_name: str): return await db_manager.get_documents(collection_name)
...
async def add_text_to_collection(text: str, collection_name: str, embedding_model: str, metadata: dict):
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        result = await asyncio.to_thread(
            genai.embed_content,
            model=f"models/{embedding_model}", content=batch_chunks, task_type="retrieval_document"
        )
        metadata_list = [metadata] * len(batch_chunks)
        chunks_added = await db_manager.add_documents(
            batch_chunks, result['embedding'], collection_name, metadata_list
        )
...
async def upload_file_api(file: UploadFile = File(...), collection_name: str = Form(...), embedding_model: str = Form(...)):
    try:
        ...
        print(f"Upload failed: {traceback.format_exc()}"); raise HTTPException(500, f"ファイル処理中にエラー: {e}")
...
async def scrape_website_api(req: ScrapeRequest):
    try:
        ...
        print(f"Scrape failed: {traceback.format_exc()}"); raise HTTPException(500, f"Webサイト処理中にエラー: {e}")
...
async def update_settings_api(req: SettingsUpdateRequest):
    ...
    await settings_manager.update_settings(req.dict()); return {"message": "Settings updated"}

@app.get("/logs", dependencies=[Depends(require_sgu_member)])
async def get_logs_api(): return await db_manager.get_logs()

@app.post("/feedback")
async def handle_feedback_api(req: FeedbackRequest):
    if await db_manager.update_feedback(req.log_id, req.rating): return {"message": "Feedback received"}
    raise HTTPException(404, "Log ID not found")

# --- WebSocket ---
@app.websocket("/ws/settings")
async def websocket_settings_endpoint(websocket: WebSocket):
    if not websocket.cookies.get("session") or not APP_SECRET_KEY: await websocket.close(1008); return
