...
- **管理者向けWebSocket**: `/ws/settings` など管理者専用 WebSocket には **接続前に GET /api/admin/system/ws-token でトークンを取得し、`?token=xxx` で接続**すること。トークン検証なしで接続を許可しない。
...
- **参照元**: 秘密鍵は main.py やルーターで `os.getenv` を直接読まず、**core.config で定義した定数**（SECRET_KEY, APP_SECRET_KEY）を使用すること。
...
❌ **学生向けAPIの認証なし**: チャット送信・履歴取得・フィードバック送信などログイン必須にすべきエンドポイントに require_auth_client を付けない実装を追加しないこと。
...
❌ **API リクエストボディで request: dict**: リクエストは必ず Pydantic の BaseModel で受け取ること。

❌ **本番で必須環境変数の Fail Fast なし**: 本番起動時に GEMINI_API_KEY, SUPABASE_URL, SUPABASE_SERVICE_KEY 等が欠落している場合、ValueError で即停止すること。
...
## 8. コード生成時に避けること（脆弱性防止）

以下のパターンは**絶対に生成しない**。監査で「要改善」となる原因となる。
...
- **本番で必須の環境変数をチェックしない**: 本番（RENDER 等）では **GEMINI_API_KEY, SUPABASE_URL, SUPABASE_SERVICE_KEY, APP_SECRET_KEY** などを lifespan 内で検証し、**欠落時は `raise ValueError(...)` で即座にプロセスを停止**する。
- **main.py やルーターで `os.getenv` / `os.environ.get` を直接使わない**: ポート番号なども **core.config に定数（例: PORT）を定義し、そこから参照**する。

### 8.4 可用性（DoS 対策）
- **ユーザー入力に長さ制限を付けない**: チャット質問・コメント・検索クエリ・URL・コレクション名など、**すべて Pydantic の `max_length`（および必要なら `min_length`）を設定**する。例: `comment: Field(None, max_length=2000)`, `url: Field(..., max_length=2048)`。
- **メモリ枯渇を考慮しない**: セッションやキャッシュには **最大件数と LRU クリーンアップ**を実装する。

### 8.5 参照
- 監査結果の詳細: `docs/SECURITY_AUDIT_REPORT.md`
- 既知の脆弱性と修正方針: `docs/SECURITY_VULNERABILITY_REPORT.md`
